...
namespace SignalCpf.Net.Tls;

/// <summary>
/// Trusts the Signal Messenger root CA from Signal-Desktop <c>config/default.json</c>.
/// Official hosts present chains that fail OS trust with PartialChain without this CA.
/// </summary>
public static class SignalCertificateAuthority
{
    // Exact PEM from signalapp/Signal-Desktop config/default.json → certificateAuthority
    private static readonly string Pem = string.Join('\n',
        "-----BEGIN CERTIFICATE-----",
    ...
    public static X509Certificate2 RootCertificate => Root.Value;

    public static bool ShouldUseSignalCa(SignalServerOptions options) =>
        options.IsOfficialLike || HostLooksLikeSignal(options.ApiBaseUrl);
        ...
        SslPolicyErrors sslPolicyErrors)
    {
        ...
        return BuildTrustedChain(leaf);
    }

    public static RemoteCertificateValidationCallback CreateCallback() => Validate;

    private static bool BuildTrustedChain(X509Certificate2 leaf)
    {
        using var customChain = new X509Chain();
        customChain.ChainPolicy.TrustMode = X509ChainTrustMode.CustomRootTrust;
        customChain.ChainPolicy.CustomTrustStore.Add(RootCertificate);
    ...
    private static bool HostLooksLikeSignal(string url)
    {
        if (!Uri.TryCreate(url, UriKind.Absolute, out var uri))
            return false;
        return uri.Host.Equals("chat.signal.org", StringComparison.OrdinalIgnoreCase)
               || uri.Host.EndsWith(".signal.org", StringComparison.OrdinalIgnoreCase);
    }
}
